          ...
          reply({
            ...
            studentList: res.map(element => {
              return {
                ...
                dob: element.dob,
                univ_school: element.univ_school,
                bio: element.bio,
                picture: element.picture,
                job_cat: element.job_cat
              };
            })
          });
        }
  ...
  server.route({
    ...
    handler: (request, reply) => {
      ...
      data.submitApplication(request.payload.job_id, (err, res) => {
        if (err) {
          reply(
            Boom.serverUnavailable('Failed to retrieve data from database')
          );
        } else {
  ...
  server.route({
    ...
    handler: (request, reply) => {
      ...
      data.loginRequest(email, (err, res) => {
        if (err) {
          return reply(Boom.unauthorized('Please log-in to see that'));
        }
        const user = res;
        comparePassword(
